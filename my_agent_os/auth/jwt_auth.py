...
def _secret_bytes() -> bytes:
    s = (settings.JWT_SECRET or os.getenv("JWT_SECRET") or "").strip() or "change-me-in-prod"
    return s.encode()
...
def encode_token(payload: dict[str, Any], expire_in: int = _EXPIRE) -> str:
    ...
    sec = _secret_bytes()
    sig = _b64url_encode(hmac.new(sec, sig_input, hashlib.sha256).digest())
...
def decode_token(token: str) -> dict[str, Any]:
    ...
    sec = _secret_bytes()
    expected = _b64url_encode(hmac.new(sec, sig_input, hashlib.sha256).digest())
